...
def read_nes_gamelist(es_gamelist_full_path):
    target_dir = os.path.join(os.getcwd(), "new")
    # for file in os.listdir(target_dir):
    #    os.rename(os.path.join(target_dir, file), file.split("=-")[-1])
    try:
        if os.path.exists(es_gamelist_full_path):
            dir_name = os.path.dirname(os.path.abspath(es_gamelist_full_path))
            logger.debug("Reading ES game list file <%s>" % es_gamelist_full_path)
            dom = read_xml(es_gamelist_full_path)
            gameList = dom.documentElement
            for game_element in gameList.getElementsByTagName("game"):
                ...
                if os.path.exists(game.path):
                    try:
                        new_file = os.path.join(target_dir, game.label) + ".zip"
                        copyfile(game.path, new_file)
                    except Exception:
                        pass
                RETRO_ARCH_LIST.items.append(game)
    except Exception:
